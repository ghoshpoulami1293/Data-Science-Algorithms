...
class my_GA:
    ...
    def evaluate(self, decision):
        ...
        if decision not in self.evaluated:
            ...
            objs_crossvalidation = []
            for eachfold in range(self.crossval_fold):
                start = int(eachfold * size)
                end = min(int((eachfold + 1) * size), len(self.data_y))
                ...
                objs = np.array(self.obj_func(
                    predictions, actuals, pred_proba))
                objs_crossvalidation.append(objs)
            # Take a mean of each fold of the cross validation result
            # objs_crossvalidation should become an 1-d array of the same size as objs
            objs_crossvalidation = np.mean(objs_crossvalidation, axis=0)
            self.evaluated[decision] = objs_crossvalidation
    ...
    def compete(self, pf_new, pf_best):
        ...
        for i, y in enumerate(pf_best):
            for j, x in enumerate(pf_new):
                domination_status = self.is_better(x, y)
                if domination_status == 1:
                    pf_best[i] = x
                    to_remove.append(j)
                    modified = True
                elif domination_status == -1:
                    to_remove.append(j)

        pf_new = [pf_new[j] for j in range(len(pf_new)) if j not in to_remove]
